...
<head>
    ...
    <script type="text/javascript">
        function comment() {
            ...
            var url = serverUrl + '/user/fileInfoComment-${requestScope.ThisFile.fileId}';
            $.post(url, {
                fileInfoComment: fileInfoComment
            }, function (requestData) {
                window.location.href="${ctx}/user/fileInfo-${requestScope.ThisFile.fileId}";
            });
            return false;
        }
                    ...
                    <div class="col-lg-5">
                        ...
                        <section class="panel">
                            <div class="twt-feed blue-bg">
                                <h1>
                                    <c:set value="${fn:length(requestScope.ThisFile.fileName) - 36}"
                                           var="length"></c:set>
                                    <p class="user">${fn:substring(requestScope.ThisFile.fileName, 0, length)}</p>
                                </h1>
                                <p>.${requestScope.ThisFile.fileType}</p>
                                <a href="#">
                                    <img src="${ctx}/resources/img/prefix/${requestScope.ThisFile.fileType}.png">
                                </a>
                            </div>
                            <div style="margin-top: 55px;">
                                <ul class="nav nav-pills nav-stacked">
                                    <li><a href="#"> <i class="fa fa-bookmark"></i> 编号：${requestScope.ThisFile.fileId}
                                    </a></li>
                                    <li><a href="#"> <i class="fa fa-user"></i> 作者：${sessionScope.loginUser.userName}
                                    </a>
                                    </li>
                                    <li><a href="#"> <i class="fa fa-puzzle-piece"></i>
                                        大小：${requestScope.ThisFile.fileSize}</a></li>
                                    <li><a href="#"> <i class="fa fa-clock-o"></i>
                                        上传时间：${fn:substring(requestScope.ThisFile.fileLoadTime, 0, 19)}</a></li>
                                    <li><a href="#"> <i class="fa fa-key"></i> 密级等级：
                                        <c:choose>
                                            <c:when test="${requestScope.ThisFile.fileSecretLevel == 5}"> A级</c:when>
                                            <c:when test="${requestScope.ThisFile.fileSecretLevel == 4}"> B级</c:when>
                                            <c:when test="${requestScope.ThisFile.fileSecretLevel == 3}"> C级</c:when>
                                            <c:when test="${requestScope.ThisFile.fileSecretLevel == 2}"> 内部</c:when>
                                            <c:otherwise> 普通</c:otherwise>
                                        </c:choose></a></li>
                                    <li><a href="#"> <i class="fa fa-eye"></i>
                                        查看次数：${requestScope.ThisFile.fileViewtimes}</a></li>
                                    <li><a href="#"> <i class="fa fa-print"></i>
                                        打印次数：${requestScope.ThisFile.filePrinttimes}</a></li>
                                    <li><a href="#"> <i class="fa fa-download"></i>
                                        下载次数：${requestScope.ThisFile.fileDownloadtimes}</a></li>
                                </ul>
                            </div>
                            <div class="twt-write col-sm-12">
                                <textarea class="form-control  t-text-area" rows="2"
                                          placeholder="${requestScope.ThisFile.fileAbstrcat}"></textarea>
                            </div>
                            <footer class="twt-footer">
                                <c:choose>
                                    <c:when test="requestScope.ThisFile.fileShare == 0">
                                ...
                                <button class="btn btn-space btn-white" data-toggle="button" style="margin-top: 15px"
                                        onclick="window.location.href='${ctx}/user/aboutUser-${sessionScope.loginUser.userId}'">
                    ...
                    <div class="col-lg-6">
                        <section class="panel">
                            <header class="panel-heading">
                                ${requestScope.ThisUser.userName}的评论
                            </header>
                                ...
                                <div class="timeline-messages">
                                    ...
                                    <div class="msg-time-chat">
                                        <a href="#" class="message-img"
                                           onclick="window.location.href='${ctx}/user/aboutUser-${requestScope.ThisUser.userId}'">
                                            <img class="avatar"
                                                 src="${requestScope.ThisUser.userPictureUrl}"></a>
                                        <div class="message-body msg-in">
                                            ...
                                            <div class="text">
                                                <p class="attribution">
                                                    <a href="#"
                                                       onclick="window.location.href='${ctx}/user/aboutUser-${requestScope.ThisUser.userId}'">
                                                        ${requestScope.ThisUser.userName}</a>
                                                    At&nbsp;&nbsp;&nbsp;&nbsp;${fn:substring(requestScope.ThisComment.comTime, 0, 19)}
                                                </p>
                                                <p>${requestScope.ThisComment.comData}</p>
                                            </div>
                                        </div>
                                    </div>
                                    <!-- /comment -->
